        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.loans.index') }}"
                    class="inline-flex items-center gap-2 px-4 py-2.5 bg-yellow-400 text-yellow-900 rounded-lg hover:bg-yellow-500 transition-smooth font-bold text-sm">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
                    Pending Requests
                    @if(\App\Models\Loan::where('status', 'pending')->count() > 0)
                        <span class="bg-white text-yellow-900 text-xs py-0.5 px-1.5 rounded-full shadow-sm">{{ \App\Models\Loan::where('status', 'pending')->count() }}</span>
                    @endif
                </a>

                <a href="{{ route('admin.loans.active') }}"
                    class="inline-flex items-center gap-2 px-4 py-2.5 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-smooth font-bold text-sm">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
                    Active Loans
                    @if(\App\Models\Loan::where('status', 'approved')->whereNull('returned_date')->count() > 0)
                        <span class="bg-white text-green-900 text-xs py-0.5 px-1.5 rounded-full shadow-sm">{{ \App\Models\Loan::where('status', 'approved')->whereNull('returned_date')->count() }}</span>
                    @endif
                </a>
