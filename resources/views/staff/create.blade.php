...
@section('page-subtitle', 'Create a new staff profile')
...
<div class="max-w-2xl">
    <form method="POST" action="{{ route('staff.store') }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-900 mb-2">Position <span class="text-red-500">*</span></label>
                <input type="text" name="position" value="{{ old('position') }}" 
                       placeholder="Librarian, Director, etc." 
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"
                       required>
                @error('position')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div>
                ...
                <textarea name="bio" rows="4" 
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500">{{ old('bio') }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" name="is_published" id="is_published" value="1" checked class="w-4 h-4 rounded border-gray-300">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('staff.index') }}" class="px-6 py-2.5 border border-gray-300 rounded-lg hover:bg-gray-50 transition-smooth font-medium text-gray-700">
                Cancel
            </a>
        </div>
    </form>
