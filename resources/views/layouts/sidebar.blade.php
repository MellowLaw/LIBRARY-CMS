<div class="w-64 bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 text-white flex flex-col">
        ...
        <div class="flex items-center gap-3 mb-2">
            <div
                ...
                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path
                        d="M12 0C5.37 0 0 5.37 0 12c0 6.63 5.37 12 12 12s12-5.37 12-12S18.63 0 12 0zm-2 18l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 10z" />
                </svg>
            </div>
            <div>
    ...
    <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
        ...
        <a href="{{ route('news.index') }}"
            class="flex items-center gap-3 px-4 py-3 rounded-lg transition-smooth {{ request()->routeIs('news.*') ? 'bg-emerald-600 text-white' : 'text-gray-300 hover:bg-gray-700' }}">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="border-t border-gray-700 px-4 py-4 space-y-2">
        ...
        <form method="POST" action="{{ route('logout') }}" class="w-full">
            @csrf
            <button type="submit"
                class="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-gray-300 hover:bg-red-900/20 transition-smooth text-left">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
