...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        <a href="{{ route('staff.index') }}"
            class="block bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-smooth cursor-pointer group">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-500 font-medium group-hover:text-purple-600 transition-colors">Staff Members
                    </p>
                ...
                <div
                    ...
                    <svg class="w-6 h-6 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 4.354a4 4 0 110 8.048M12 4.354L9.172 7.172A4 4 0 0012.828 16.83m0-12.496h.028m9.026 9.142c-3.900 3.900-10.236 3.900-14.142 0M15.763 19.9a6.009 6.009 0 01-8.486 0m11.334-11.668c1.886-1.886 1.886-4.944 0-6.83-1.886-1.886-4.944-1.886-6.83 0m6.83 6.83l-6.83-6.83">
                        </path>
                    </svg>
                </div>
            </div>
        ...
        <a href="{{ route('books.index') }}"
            ...
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-500 font-medium group-hover:text-indigo-600 transition-colors">Total Books
                    </p>
            ...
            @if($myLoans->isNotEmpty())
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    <div class="p-6 border-b border-gray-200 flex items-center justify-between">
                        <h3 class="text-lg font-bold text-gray-900">My Borrowed Books</h3>
                        <span
                            class="px-2 py-1 bg-indigo-100 text-indigo-700 text-xs font-bold rounded-full">{{ $myLoans->count() }}
                            Active</span>
                    </div>
                    <div class="divide-y divide-gray-200">
                        @foreach($myLoans as $loan)
                            <div class="p-6 flex items-center justify-between hover:bg-gray-50 transition-smooth">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 class="text-sm font-bold text-gray-900">{{ $loan->book->title }}</h4>
                                        <p class="text-xs text-gray-500 mt-1">Due on {{ $loan->due_date->format('M d, Y') }}</p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-3">
                                    <div
                                        class="text-xs font-medium {{ $loan->due_date->isPast() ? 'text-red-600' : 'text-gray-500' }}">
                                        {{ $loan->due_date->diffForHumans() }}
                                    </div>
                                    <a href="{{ route('public.books.show', $loan->book) }}"
                                        class="px-3 py-1 bg-indigo-50 text-indigo-600 text-xs font-bold rounded-lg hover:bg-indigo-100 transition-smooth">
                                        View Book
                                    </a>
                                </div>
                            </div>
                ...
                <div class="divide-y divide-gray-200">
                    ...
                    @empty
                        <div class="p-6 text-center text-gray-500">
                            No pages yet. <a href="{{ route('pages.create') }}"
                                class="text-emerald-600 hover:text-emerald-700 font-medium">Create one</a>
                        </div>
                    @endforelse
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="space-y-3">
                <a href="{{ route('pages.create') }}"
                    class="flex items-center gap-3 w-full p-3 bg-blue-50 hover:bg-blue-100 rounded-lg transition-smooth text-blue-700 font-medium">
                ...
                <a href="{{ route('news.create') }}"
                    class="flex items-center gap-3 w-full p-3 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition-smooth text-emerald-700 font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('staff.create') }}"
                    class="flex items-center gap-3 w-full p-3 bg-purple-50 hover:bg-purple-100 rounded-lg transition-smooth text-purple-700 font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('books.create') }}"
                    class="flex items-center gap-3 w-full p-3 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-smooth text-indigo-700 font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253">
                        </path>
                ...
                <a href="{{ route('menus.index') }}"
                    class="flex items-center gap-3 w-full p-3 bg-amber-50 hover:bg-amber-100 rounded-lg transition-smooth text-amber-700 font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
